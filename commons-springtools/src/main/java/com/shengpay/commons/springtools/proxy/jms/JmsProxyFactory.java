...
public class JmsProxyFactory implements MessageListener,FactoryBean,InvocationHandler,BeanClassLoaderAware,BeanFactoryAware {
	 ...
	 * JMS属性名:应答目标服务器地址信息
	 */
	private static final String JMS_PROPERTY_NAME_RESPONSE_AIM_HOST = "JMS_PROPERTY_NAME_RESPONSE_AIM_HOST";
	
	/**
	 * JMS属性名:发送者ID地址
	 */
	private static final String JMS_PROPERTY_NAME_PRODUCER_ID = "JMS_PROPERTY_NAME_PRODUCER_ID";
	
	/**
	 * JMS属性名:消费者ID地址
	 */
	private static final String JMS_PROPERTY_NAME_CONSUMER_ID = "JMS_PROPERTY_NAME_CONSUMER_ID";
	...
	private void initThreadPoolExecutor() {
		threadPoolExecutor=new ThreadPoolExecutor(threadPoolSize,threadPoolSize,60,TimeUnit.SECONDS,new ArrayBlockingQueue<Runnable>(1000),new ThreadPoolExecutor.AbortPolicy());
	}
	...
	public void init() {
		...
		//取得链接工厂和消息队列
		connectionFactory = (ConnectionFactory) getObjectByJndi(connectionFactoryJndi);
	...
	private String appendNpaFilter(String classFilterStr, NativeProcessAnn classNativeProcessAnn) {
		if(classNativeProcessAnn!=null) {
			String npaFilterStr=JMS_PROPERTY_NAME_RESPONSE_AIM_HOST+"='"+getLocalIp()+"'";
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		Object[] newArgs=makeNewArgs(args,method);
		
		// 使用会话创建消息
		TextMessage objMessage = jmsSession.createTextMessage();
		objMessage.setText(JsonUtils.toJson(newArgs));
		objMessage.setStringProperty(JMS_PROPERTY_NAME_METHOD_SIGN_FULL, methodSignFull);// 发布刷新文章消息
		
		//设置其他方法上的jms属性信息
		serOtherMsgProperties(method, newArgs, objMessage);
		serNativeProcessProperties(method, args, objMessage);
		
		// 使用消息生产者发送消息
		producer.send(objMessage);
		logger.info("成功对方法调用【" + ClassUtils.getMethodCallInfo(method, newArgs) + "】进行了JMS消息发送代理【" + objMessage.getJMSMessageID() + "】");
		...
		//如果返回值为异常信息,则抛出,否则返回
		Object[] objArr = JsonUtils.fromJson(new Class[] { method.getReturnType() }, returnMessage.getText());
	 ...
	 * @throws JMSException 
	 */
	private void serNativeProcessProperties(Method method, Object[] newArgs, TextMessage objMessage) throws JMSException {
		Class<?>[] parameterTypes = method.getParameterTypes();
		for (int i = 0; i < parameterTypes.length; i++) {
			Class<?> class1 = parameterTypes[i];
			Object arg=newArgs[i];
			if(class1 == NativeProcessContext.class && arg!=null) {
				String aimHost=((NativeProcessContext)arg).getLocalHost();
				setMessageProperty(objMessage, JMS_PROPERTY_NAME_RESPONSE_AIM_HOST, aimHost);
			}
		}
	 ...
	 * @return
	 */
	private Object[] makeNewArgs(Object[] args, Method method) {
		...
		for (int i = 0; i < parameterTypes.length; i++) {
			...
			if(class1==NativeProcessContext.class && args[i]==null) {
				newArgs[i]=makeNativeProcessContext();
			}else {
				newArgs[i]=args[i];
			}
		}
	 ...
	 * @return
	 */
	private NativeProcessContext makeNativeProcessContext() {
		NativeProcessContext npc=new NativeProcessContext();
		return npc;
	}
	 ...
	 * @throws JMSException
	 */
	private void serOtherMsgProperties(Method method, Object[] args, TextMessage objMessage) throws IllegalAccessException, JMSException {
	...
	/**
	 * 设置【{@link #threadPoolSize threadPoolSize}】
	 * @param 类型：int
	 */
